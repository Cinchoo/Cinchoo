...
{
    public class ChoShellScriptPlugIn : ChoPlugIn
    {
        ...
        protected override object Execute(object value, out bool isHandled)
        {
            ...
            _tmpFileName = Path.ChangeExtension(ChoPath.GetTempFileName(), ChoReservedFileExt.BAT);
            File.WriteAllText(_tmpFileName, script);
            ChoTrace.DebugFormat("{0}: Temp file created at '{1}'", Name, _tmpFileName);

            using (ChoProcess p = new ChoProcess(_tmpFileName, arguments))
            {
        ...
        public override void InitializeBuilder(ChoPlugInBuilder builder)
        {
            ...
            ChoShellScriptPlugInBuilder b = builder as ChoShellScriptPlugInBuilder;
