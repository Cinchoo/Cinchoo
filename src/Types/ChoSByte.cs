namespace Cinchoo.Core
{
    ...
    public class ChoSByte : IChoStringObjectFormatter<sbyte>
    {
        ...
        private static readonly Regex _regEx = new Regex(String.Format(@"^(?<value>\d+){0}$", Postfix), RegexOptions.Compiled);
        ...
        public ChoSByte(sbyte value)
        {
            _value = value;
        }

        public ChoSByte(string formattedValue)
        {
            if (formattedValue == null || formattedValue.Length == 0)
                throw new ArgumentNullException("formattedValue");

            _value = (sbyte)Parse(formattedValue);
        }
        ...
        public static bool TryParse(string value, out sbyte output)
        {
            output = 0;
            ...
            if (IsSByte(value))
                return SByte.TryParse(GetValue(value), out output);
            else
                return false;
        }
        ...
        public static implicit operator sbyte(ChoSByte sbyteObject)
        {
            return sbyteObject == null ? (sbyte)0 : sbyteObject.Value;
        }
