namespace eSquare.Core.Diagnostics.Logging
{
    ...
    public static class ChoLogger
    {
        ...
        static ChoLogger()
        {
            ChoStreamProfile.Clean(ChoLogDirectories.Settings, typeof(ChoLogger).Name);
        }
        ...
        internal static void Log(StackFrame callerStackFrame, object message)
        {
            ...
            foreach (IChoLogManager logManager in ChoLogManagerSettings.Me.LoggerManagers)
            {
                ...
                {
                    ChoStreamProfile.WriteLine(ChoLogDirectories.Settings, typeof(ChoLogger).Name, String.Format("[{0}]: {1}", logManager.GetType().Name, ex.Message));
                }
            }
        }
