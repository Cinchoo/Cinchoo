namespace Cinchoo.Core.Collections.Generic
{
	...
	public class ChoConcurrentDictionary<TKey, TValue>
		: IDictionary<TKey, TValue>, IDictionary
	{
		...
		[NonSerialized]
		private volatile ChoNode<TKey, TValue>[] _buckets;
		...
		// Methods
		public ChoConcurrentDictionary()
			: this(DefaultConcurrencyLevel, DEFAULT_CAPACITY)
		{
		}

		public ChoConcurrentDictionary(IEnumerable<KeyValuePair<TKey, TValue>> collection)
			: this(collection, EqualityComparer<TKey>.Default)
		{
		}

		public ChoConcurrentDictionary(IEqualityComparer<TKey> comparer)
			: this(DefaultConcurrencyLevel, DEFAULT_CAPACITY, comparer)
		{
		}

		public ChoConcurrentDictionary(IEnumerable<KeyValuePair<TKey, TValue>> collection, IEqualityComparer<TKey> comparer)
			: this(DefaultConcurrencyLevel, collection, comparer)
		{
		}

		public ChoConcurrentDictionary(int concurrencyLevel, int capacity)
			: this(concurrencyLevel, capacity, EqualityComparer<TKey>.Default)
		{
		}

		public ChoConcurrentDictionary(int concurrencyLevel, IEnumerable<KeyValuePair<TKey, TValue>> collection, IEqualityComparer<TKey> comparer)
		...
		public ChoConcurrentDictionary(int concurrencyLevel, int capacity, IEqualityComparer<TKey> comparer)
		{
			...
			_buckets = new ChoNode<TKey, TValue>[capacity];
			_comparer = comparer;
		}
		...
		{
			return key; // Environment.GetResourceString(key);
		}
		...
		private void OnDeserialized(StreamingContext context)
		{
			...
			_buckets = new ChoNode<TKey, TValue>[_serializationCapacity];
		...
		public void Clear()
		{
			...
			{
				AcquireAllLocks(ref locksAcquired);
				_buckets = new ChoNode<TKey, TValue>[DEFAULT_CAPACITY];
		...
		#region ConcurrentDictionaryEnumerator Class
		...
		private sealed class ConcurrentDictionaryEnumerator : IEnumerator<KeyValuePair<TKey, TValue>>, IEnumerator, IDisposable
		{
			...
			private ChoConcurrentDictionary<TKey, TValue> _dictionary;
			private ChoNode<TKey, TValue>[] _buckets;
			private ChoNode<TKey, TValue> _currentNode;
			private int _index;
		...
		#endregion ConcurrentDictionaryEnumerator Class

		#region DictionaryEnumerator Class

		private class DictionaryEnumerator<TKey1, TValue1>
			: IDictionaryEnumerator
		{
			// Fields
			private readonly IEnumerator<KeyValuePair<TKey1, TValue1>> _enumerator;
		...
		#endregion DictionaryEnumerator Class
	}
}