namespace Cinchoo.Core.Collections.Generic
{
    ...
    {
        [Serializable]
        private class ChoUniqueList<T1> : ChoList<T1>, ICloneable
        {
            ...
            {
                return new ChoUniqueList<T1>(base.Clone() as ChoList<T1>);
            }
            ...
            private bool IsItemUnique(T1 item)
            {
                bool isUnique = !_innerList.Contains(item);
                if (!isUnique && !_silent)
