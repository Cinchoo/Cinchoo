namespace Cinchoo.Core.Collections.Generic
{
    ...
    {
        [Serializable]
        private class ChoNotNullableList<T1> : ChoList<T1>, ICloneable
        {
            ...
            {
                return new ChoNotNullableList<T1>(base.Clone() as ChoList<T1>);
            }
            ...
            private bool IsItemNull(T1 item)
            {
                bool isNull = item == null;

                if (isNull && !_silent)
                    throw new ApplicationException("Item is null.");
